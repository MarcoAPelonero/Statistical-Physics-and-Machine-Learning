...
public:
    ...
    double evalProd(const Vector<int>& S) const {
        assert(S.getSize() == N);
        double acc = 0.0;
        for (int i = 0; i < N; ++i) acc += static_cast<double>(S[i]) * J_[i];
        return acc;
    ...
    void setWeights(const std::array<double, N>& newWeights) {
        for (int i = 0; i < N; ++i) {
            J_[i] = newWeights[i];
        }
    }
    ...
    // Weight wrapper struct for mutable access to weights
    ...
    // Get mutable access to weights for instant learning rules
    WeightWrapper getWeights() {
        return WeightWrapper{J_, N};
    }

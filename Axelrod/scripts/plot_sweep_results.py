...
Usage examples:
  python scripts/plot_sweep_results.py --network smallworld
  python scripts/plot_sweep_results.py --network lattice
  python scripts/plot_sweep_results.py --network lattice --data-dir data/lattice --out-dir figures/lattice
...
def create_comparison_figure(sweeps, data_dir, out_dir):
    ...
    for idx, (fname, label) in enumerate(sweeps):
        if idx >= len(axes):
            break
        df = read_sweep_csv(os.path.join(data_dir, fname))
        if df is None:
            continue
...
def main():
    ...
    p.add_argument('--network', choices=['smallworld', 'lattice'], default='smallworld',
                   help='Which network results to plot (matches data/<network>).')
    p.add_argument('--data-dir', help='Directory with sweep CSV files (default: data/<network>)')
    p.add_argument('--out-dir', help='Output directory for PNG files (default: figures/<network>)')
    ...
    out_dir = args.out_dir or os.path.join('figures', args.network)
    os.makedirs(out_dir, exist_ok=True)

    # List of sweeps to plot depending on network type
    if args.network == 'smallworld':
        sweeps = [
            ('sweep_neighbors_per_node.csv', 'Neighbors per Node'),
            ('sweep_rewiring_prob.csv', 'Rewiring Probability'),
            ('sweep_num_features.csv', 'Number of Features'),
            ('sweep_feature_dim.csv', 'Feature Dimension'),
        ]
    else:
        sweeps = [
            ('sweep_lattice_radius.csv', 'Lattice Radius'),
            ('sweep_num_features.csv', 'Number of Features'),
            ('sweep_feature_dim.csv', 'Feature Dimension'),
        ]
    ...
    for fname, param_name in sweeps:
        df = read_sweep_csv(os.path.join(data_dir, fname))
        if df is None:
            continue
        ...
        create_2x2_subplot(param_name, param_values, df, out_dir)

    # Create comparison figure
    print("\nCreating comparison figure...")
    create_comparison_figure(sweeps, data_dir, out_dir)
